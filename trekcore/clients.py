...
class Client:
	...
	def load_shipmodule(self,m):
		...
		return [u8(m["type"]), u8(m["status"])]+module_name+[u8(m["stats"]["health"]["current"]), u8(m["stats"]["health"]["max"])]+[u8(m["stats"]["power"]["draw"]), u8(m["stats"]["power"]["required"])]+self.load_module_sprite(m["icon"])
