import os,yaml,logging,sys,gzip,datetime,traceback,socket,threading,ctypes,hashlib,random
	...
	def load_config(self):
		with open(f"server.properties", "r") as f:
			...
			if self.config["security"]["rsa_keylen"] not in range (1024, 2048):
				raise Exception("RSA key length invalid. Must be in range 1024-2048.")
			if self.config["security"]["aes_keylen"] not in range (128, 256, 64):
				raise Exception("AES key length invalid. Must be 128, 192, or 256.")
...
#####################################
	...
	def send_bytes(self, data):
		try:
			...
			if len(data)+3 > Client.config["packet-max"]:
				raise Exception(f"Send error, Packet ID {data[0]}: Packet + size header exceeds packet max spec.")
	...
	def listener(self):
		...
		try:
			...
			while Client.server.online and self.connected:
				...
				if not data: break		# conn.recv returning nothing means conn closed?
				...
				if not self.data_size:
					if len(self.in_buf) < 3: continue	# if no size word, just skip back to recv()
	...
	def login(self, data):
		try:
			...
			hmac_verify = hmac_new(self.hmac_key, iv + ct, hashlib.sha256).digest()
			if not hmac.compare_digest(hmac_digest, hmac_verify):
				raise LoginError("HMAC validation error")
			
			cipher = AES.new(self.aes_key, AES.MODE_CTR, nonce=iv[:8], initial_value=iv[8:])
